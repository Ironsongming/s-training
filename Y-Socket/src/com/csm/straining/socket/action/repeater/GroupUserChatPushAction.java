 ...
 * @author chensongming
 */
public class GroupUserChatPushAction extends ActionSupport{
	
	private static final Logger logger = LoggerFactory.getLogger(GroupUserChatPushAction.class);
	...
	protected void execute() throws ActionException {
		...
		if (session == null) {
			logger.info("GroupUserChatPushAction no target userID");
			return;
		}
		
		Message msg = MessageUtil.getMessage(MessageCode.GroupChatPushPID.REQUEST, JSON.toJsonString(message.getMsg()));
		
		try {
			session.sendMessage(msg);
		} catch (Exception e) {
			logger.error("GroupUserChatPushAction error");
		}
	}
